...
export function EditableNote({
  ...
  const handleUpdateScore = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        title: "Score Updated",
        description: `Note score has been set to ${scoreValue}.`,
      });
  ...
  return (
    <div className="bg-muted p-4 rounded-lg space-y-4">
      ...
      <div className="text-sm text-muted-foreground">
        <p>
          Author: {note.authorPseudonym} ({note.authorUid})
        </p>
...
export function ReportsTab() {
  ...
  const handleDismissReport = async (reportId: string, noteId: string) => {
    try {
      ...
      toast({
        title: "Error",
        description: "Could not dismiss the report.",
        variant: "destructive",
      });
    }
  };
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Notes that have been flagged by users for review.</CardDescription>
      </CardHeader>
        ...
        {reports.map((report) => (
          <div key={report.id} className="border p-4 rounded-lg space-y-4">
            <div>
              ...
              <p className="text-sm">
                Status: {report.noteVisibility === "unlisted" ? "Hidden" : "Pending"}
              </p>
            ...
            <div className="flex justify-end gap-2 pt-4">
              <Button
                variant="outline"
                onClick={() => handleDismissReport(report.id, report.noteId)}
              >
                Dismiss Report
              </Button>
              <Button
                variant="destructive"
                onClick={() => handleDeleteNoteFromReport(report.id, report.noteId)}
              >
                Delete Note & Report
              </Button>
            </div>
